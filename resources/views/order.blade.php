...
<main class="container mb-5 position-relative">
    ...
    <div class="row g-4">
        ...
        <div class="col-lg-8">
            @foreach($menus as $kategori => $daftarMenu)
                <div class="mb-5">
                    <h3 class="fw-bold text-kopi border-bottom pb-2 mb-4">
                        <i class="bi bi-tags-fill me-2"></i> {{ ucfirst($kategori) }}
                    </h3>

                    @foreach($daftarMenu as $menu)
                        <div class="product-card" data-id="{{ $menu->id }}" data-price="{{ $menu->harga }}" data-name="{{ $menu->nama }}">
                            <img src="{{ $menu->gambar && file_exists(public_path('uploads/menus/' . $menu->gambar)) ? asset('uploads/menus/' . $menu->gambar) : 'https://images.unsplash.com/photo-1559525839-b184a4d696c5' }}" 
                                 class="card-img-top" 
                                 alt="{{ $menu->nama }}" 
                            ...
                            <div class="product-info">
                                <div class="d-flex justify-content-between align-items-start">
                                    <div>
                                        <h4 class="product-title">{{ $menu->nama }}</h4>
                                        <p class="product-desc d-none d-sm-block">{{ $menu->deskripsi ?? 'Nikmati varian menu terbaik racikan barista kami.' }}</p>
                                    </div>
                                </div>
        ...
        <div class="col-lg-4">
            <div class="cart-sidebar">
                ...
                <div id="address-container" class="d-none mb-3">
                    <label class="form-label fw-bold text-dark mb-2">Alamat Pengiriman</label>
                    <textarea id="delivery-address" class="form-control rounded-3" rows="2" placeholder="Masukkan alamat lengkap kirim kopi..."></textarea>
                </div>
...
<script>
    document.addEventListener('DOMContentLoaded', () => {
        ...
        // Event listener ganti opsi pengiriman
        opsiPengirimanSelect.addEventListener('change', function() {
        ...
        // Jalur Checkout AJAX Kirim Data ke PesananController
        checkoutBtn.addEventListener('click', () => {
            const jenisPesanan = opsiPengirimanSelect.value;
            const alamat = deliveryAddressInput.value;

            if (jenisPesanan === 'delivery' && !alamat.trim()) {
            ...
            const payload = {
                ...
                total_tamu: rawTotal, 
                items: itemsOrdered,
                alamat_pengiriman: jenisPesanan === 'delivery' ? alamat : null,
                _token: '{{ csrf_token() }}'
            };
